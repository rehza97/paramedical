...
def upgrade() -> None:
    ...
    op.create_table('promotion_years',
        ...
        sa.Column('annee_calendaire', sa.Integer(), nullable=False),
        sa.Column('nom', sa.String(length=200), nullable=True),
        sa.Column('date_debut', sa.String(length=10), nullable=True),
    ...
    op.create_table('promotion_year_services',
        sa.Column('promotion_year_id', sa.String(length=36), nullable=False),
        sa.Column('service_id', sa.String(length=36), nullable=False),
        sa.ForeignKeyConstraint(['promotion_year_id'], ['promotion_years.id'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['service_id'], ['services.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('promotion_year_id', 'service_id')
    )
    
    # Add new columns to existing tables
    op.add_column('etudiants', sa.Column('annee_courante', sa.Integer(), nullable=False, server_default='1'))
    op.add_column('plannings', sa.Column('promotion_year_id', sa.String(length=36), nullable=True))
    op.add_column('plannings', sa.Column('annee_niveau', sa.Integer(), nullable=True))
    op.create_foreign_key(None, 'plannings', 'promotion_years', ['promotion_year_id'], ['id'])
