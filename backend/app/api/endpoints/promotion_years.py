...
def get_active_promotion_year(
    ...
    active_year = promotion_year.get_active_year(
        db=db, promotion_id=promotion_id)
    if not active_year:
        raise HTTPException(
            status_code=404, detail="Aucune année active trouvée")
    return active_year
...
def activate_promotion_year(
    ...
    return promotion_year.activate_year(db=db, year_id=promotion_year_id)
...
def get_promotion_year(
    ...
    if not promo_year:
        raise HTTPException(
            status_code=404, detail="Année de promotion non trouvée")
    return promo_year
...
def assign_service_to_promotion_year(
    ...
    if db_service in promo_year.services:
        raise HTTPException(
            status_code=400, detail="Service déjà assigné à cette année")
...
def remove_service_from_promotion_year(
    ...
    if db_service not in promo_year.services:
        raise HTTPException(
            status_code=400, detail="Service non assigné à cette année")
