...
async def create_promotion(promotion: schemas.PromotionCreate, db: Session = Depends(get_db)):
    ...
    if not promotion.etudiants or len(promotion.etudiants) == 0:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="La promotion doit contenir au moins un étudiant.")
...
async def get_promotion(promo_id: str, db: Session = Depends(get_db)):
    promotion = db.query(models.Promotion).filter(models.Promotion.id == promo_id).first()
    if not promotion:
...
async def delete_promotion(promo_id: str, db: Session = Depends(get_db)):
    ...
    plannings = db.query(models.Planning).filter(models.Planning.promo_id == promo_id).all()
    for planning in plannings:
        # Delete rotations for this planning
        db.query(models.Rotation).filter(models.Rotation.planning_id == planning.id).delete()
    
    # Then delete the plannings
    db.query(models.Planning).filter(models.Planning.promo_id == promo_id).delete()
...
async def create_speciality(speciality: schemas.SpecialityCreate, db: Session = Depends(get_db)):
    ...
    existing_speciality = db.query(models.Speciality).filter(models.Speciality.nom == speciality.nom).first()
    if existing_speciality:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Le nom de la spécialité doit être unique.")
...
async def get_speciality(speciality_id: str, db: Session = Depends(get_db)):
    speciality = db.query(models.Speciality).filter(models.Speciality.id == speciality_id).first()
    if not speciality:
...
async def update_speciality(speciality_id: str, speciality: schemas.SpecialityCreate, db: Session = Depends(get_db)):
    db_speciality = db.query(models.Speciality).filter(models.Speciality.id == speciality_id).first()
...
async def delete_speciality(speciality_id: str, db: Session = Depends(get_db)):
    ...
    promotions_count = db.query(models.Promotion).filter(models.Promotion.speciality_id == speciality_id).count()
...
async def create_service(service: schemas.ServiceCreate, db: Session = Depends(get_db)):
    ...
    if service.places_disponibles < 1 or service.duree_stage_jours < 1:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Les valeurs doivent être positives et supérieures à zéro.")
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Le nom du service doit être unique.")
...
async def generer_planning(promo_id: str, date_debut: str = "2025-01-01", db: Session = Depends(get_db)):
    ...
    planning = generer_algorithme_repartition(promotion, services, date_debut, db)
...
async def get_planning(promo_id: str, db: Session = Depends(get_db)):
    planning = db.query(models.Planning).filter(models.Planning.promo_id == promo_id).first()
    if not planning:
        raise HTTPException(status_code=404, detail="Planning non trouvé")
    
    # Add promotion name and student/service names to rotations
    planning_dict = schemas.Planning.from_orm(planning).dict()
    planning_dict["promo_nom"] = planning.promotion.nom
    
    for rotation in planning_dict["rotations"]:
        # Get student name
        etudiant = db.query(models.Etudiant).filter(models.Etudiant.id == rotation["etudiant_id"]).first()
        if etudiant:
            rotation["etudiant_nom"] = f"{etudiant.prenom} {etudiant.nom}"
        
        # Get service name
        service = db.query(models.Service).filter(models.Service.id == rotation["service_id"]).first()
        if service:
...
async def get_planning_etudiant(promo_id: str, etudiant_id: str, db: Session = Depends(get_db)):
    ...
    for rotation in rotations:
        rotation_dict = schemas.Rotation.from_orm(rotation).dict()
...
def generer_algorithme_repartition(promotion: models.Promotion, services: List[models.Service], date_debut_str: str, db: Session) -> schemas.Planning:
    ...
    for idx_etudiant, etudiant in enumerate(etudiants):
        ...
        decalage_jours = (idx_etudiant // min_places) * 7  # Nouveau groupe tous les 7 jours
    ...
    return schemas.Planning.from_orm(db_planning)
